cbcrisk <- function(race, profile, start.age, pred.year=5, print.output=T){
  ...
  if(race==2 & length(profile)==4){return(cbcriskBlack(profile, start.age, pred.year, print.output = T))}
  if(race==2 & length(profile)!=4){print("Race provided is 2 (non-Hispanic Black). Please insert a correct patient profile for this race")}
  if(race==1 & length(profile)==8){return(cbcrisk_nonBlack(profile, start.age, pred.year, print.output = T))}
  if(race==1 & length(profile)!=8){print("Race provided is 1 (unspecified). Please insert a correct patient profile for this race")}
}
